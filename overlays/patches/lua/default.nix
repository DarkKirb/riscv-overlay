# similar to interpreters/python/default.nix
{
  ...
  passthruFun = {
    ...
    luaPackages =
      ...
      ({
        ...
        generatedPackages =
          if (builtins.pathExists "${path}/pkgs/development/lua-modules/generated-packages.nix")
          then (final: prev: callPackage "${path}/pkgs/development/lua-modules/generated-packages.nix" {inherit (final) callPackage;} final prev)
          else (final: prev: {});
  ...
  in rec {
    ...
    withPackages = import "${path}/pkgs/development/interpreters/lua-5/with-packages.nix" {inherit buildEnv luaPackages;};
    pkgs = let
      lp = luaPackages;
    in
      lp // {luaPackages = lp.luaPackages // {__attrsFailEvaluation = true;};};
...
in rec {
  luajit_2_1 = import ./luajit/luajit.nix {
    self = luajit_2_1;
    inherit callPackage fetchFromGitHub passthruFun lib;
  };
}
